  ...
  <div class="mod-role">
    ...
    <el-table
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="180" label="操作">
        <template slot-scope="scope">
          <el-button v-if="scope.row.commonStatus == 5" type="text" size="small" @click="refundHandle(scope.row.id)">
            同意退款
          </el-button>
          <el-button v-if="scope.row.commonStatus == 5" type="text" size="small"
                     @click="refusedRefundHandle(scope.row.id)">
            驳回退款
          </el-button>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      refusedRefundHandle(id){
        this.$prompt('请输入拒绝理由', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
        }).then(({value}) => {
          this.$http({
            url: this.$http.adornUrl('orderGoods/refund'),
            method: 'post',
            data: {id: id, opt: 0, message: value}
          }).then(({data}) => {
